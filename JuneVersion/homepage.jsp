...
<body>
	...
	<div class="topnav" id="myTopnav">
		...
		<div class="signoutbtn">
			<a href="#about">${loggedUsername}</a> <a href="listgames">BROWSE</a>
			<a
				href="displayuserlist?id=<c:out value='${loggedID}' />&listname=favourites">Favourites</a>
			<a
				href="displayuserlist?id=<c:out value='${loggedID}' />&listname=wanttoplay">Want
				To Play</a> <a
				href="displayuserlist?id=<c:out value='${loggedID}' />&listname=hasplayed">Has
				Played</a> <a href="http://localhost:8080/gamerlog_project/login.jsp">SIGN
				OUT</a>
		</div>
			...
			<c:forEach var="game" items="${listGame}">
				<tr>
					...
					<td><a href="newlistentry?user=<c:out value='${loggedID}' />&game=<c:out value='${game.id}' />&listname=favourites">Favourites</a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="deletegame?id=<c:out value='${game.id}' />">Want to Play</a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="deletegame?id=<c:out value='${game.id}' />">Has Played</a>
					</td>
				</tr>
			</c:forEach>
		</table>
